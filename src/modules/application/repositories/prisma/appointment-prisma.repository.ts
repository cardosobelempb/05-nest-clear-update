...
export class AppointmentPrismaRepository implements AppointmentRepository {
  ...
  async findManyByAvailableTimeId(availableTimeId: string, {page}: Pagination.Params): Promise<AppointmentEntity[]> {
    const appointments = await this.prismaService.appointment.findMany({
      where: {
        availableTimeId
      },
  ...
  async findManyByServiceId(serviceId: string, {page}: Pagination.Params): Promise<AppointmentEntity[]> {
    const appointments = await this.prismaService.appointment.findMany({
      where: {
        serviceId
      },
  ...
  async findManyByUserId(userId: string, {page}: Pagination.Params): Promise<AppointmentEntity[]> {
    const appointments = await this.prismaService.appointment.findMany({
      where: {
        userId
      },
