...
export class AvailableTimeDelete {
  constructor(private readonly appointimentRespository: AvailableTimeRepository) { }

  async execute({ userId, availabletimeId }: AvailableTimeDeleteProps.Request) {

    const availabletime = await this.appointimentRespository.findById(availabletimeId)
